...
Eigen::MatrixX4f PatchWorkpp::toEigenCloudX4f(vector<PointXYZ> cloud)
{
    Eigen::MatrixX4f dst(cloud.size(), 4);
    int j=0;
    for (auto &p: cloud) {
        dst.row(j++) << p.x, p.y, p.z, p.idx;
    }
    return dst;
}

Eigen::MatrixX3f PatchWorkpp::toEigenCloudX3f(vector<PointXYZ> cloud)
{
    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                centers_.push_back(PointXYZ(pc_mean_(0), pc_mean_(1), pc_mean_(2), 0.0f));
                normals_.push_back(PointXYZ(normal_(0), normal_(1), normal_(2), 0.0f));
...
void PatchWorkpp::reflected_noise_removal(Eigen::MatrixXf &cloud_in)
{
    ...
    for (int i=0; i<cloud_in.rows(); i++)
    {
        ...
        {
            cloud_nonground_.push_back(PointXYZ(cloud_in.row(i)(0), cloud_in.row(i)(1), cloud_in.row(i)(2), cloud_in.row(i)(4)));
...
void PatchWorkpp::pc2czm(const Eigen::MatrixXf &src, std::vector<Zone> &czm) {
    ...
    for (int i=0; i<src.rows(); i++) {
        ...
        float x = src.row(i)(0), y = src.row(i)(1), z = src.row(i)(2), idx = src.row(i)(4);
        ...
        if ((r <= max_range) && (r > min_range)) {
            ...
            if (r < min_range_1) { // In First rings
                ...
                czm[0][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, idx));
                ...
                czm[1][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, idx));
                ...
                czm[2][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, idx));
            } else { // Far!
                ...
                czm[3][ring_idx][sector_idx].emplace_back(PointXYZ(x, y, z, idx));
            }

        } else {
            cloud_nonground_.push_back(PointXYZ(x, y, z, idx));
        }
    }

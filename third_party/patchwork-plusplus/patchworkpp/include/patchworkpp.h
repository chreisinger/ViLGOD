...
struct PointXYZ {
    ...
    PointXYZ(float _x, float _y, float _z, float _idx) : x(_x), y(_y), z(_z), idx(_idx) {}
};
...
public:
    ...
    Eigen::MatrixX4f getGround() { return toEigenCloudX4f(cloud_ground_); }
    Eigen::MatrixX4f getNonground() { return toEigenCloudX4f(cloud_nonground_); }
    
    Eigen::MatrixX3f getCenters() { return toEigenCloudX3f(centers_); }
    Eigen::MatrixX3f getNormals() { return toEigenCloudX3f(normals_); }
...
private:
    ...
    Eigen::MatrixX3f toEigenCloudX3f(vector<PointXYZ> cloud);
    Eigen::MatrixX4f toEigenCloudX4f(vector<PointXYZ> cloud);

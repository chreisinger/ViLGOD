...
# adaptation for configurable thresholds and range breakdowns
...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def build_config(self, difficulties=[1, 2], breakdown_range=True, iou_thresholds=[0.7, 0.5, 0.5, 0.5], **kwargs):
        ...
        for threshold in iou_thresholds:
            config.iou_thresholds.append(threshold)
    ...
    def build_graph(self, graph, cfg):
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True, cfg={}):
        ...
        graph = tf.Graph()
        metrics = self.build_graph(graph, cfg=cfg)
